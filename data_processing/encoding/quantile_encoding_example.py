...
def quantile_dates_and_numeric(df,
                            path_for_PreFer_folder,
                            static_columns = [ # cols that should not be touched by encoding
                                            'nomem_encr',
                                            'new_child',
                                            'outcome_available',
                                             ]
    ...
    codebook = pd.read_csv(path_for_PreFer_folder + '/codebooks/PreFer_codebook.csv')
...
if __name__ == '__main__':
    ...
    df = pd.read_csv(path_for_PreFer_folder + '/training_data/PreFer_train_data.csv',
                     low_memory=False)
    # transform all date and numeric columns to quantiles (drop cols that could not be transformed)
